...
#include "include/ArduPilotPlugin_Rover.hh"

#define MAX_MOTORS 255

using namespace gazebo;

GZ_REGISTER_MODEL_PLUGIN(ArduPilotPlugin_Rover)
...
/////////////////////////////////////////////////
ArduPilotPlugin_Rover::ArduPilotPlugin_Rover()
...
/////////////////////////////////////////////////
void ArduPilotPlugin_Rover::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
...
*/
  ...
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
      std::bind(&ArduPilotPlugin_Rover::OnUpdate, this));
...
/////////////////////////////////////////////////
void ArduPilotPlugin_Rover::OnUpdate()
{
...
/////////////////////////////////////////////////
void ArduPilotPlugin_Rover::ResetPIDs()
{
...
/////////////////////////////////////////////////
bool ArduPilotPlugin_Rover::InitArduPilotSockets(sdf::ElementPtr _sdf) const
{
...
/////////////////////////////////////////////////
void ArduPilotPlugin_Rover::ApplyMotorForces(const double _dt)
{
  ...
  for (size_t i = 0; i < this->dataPtr->controls.size(); ++i)
  {
    ...
    else 
    {
      ...
      else if (this->dataPtr->controls[i].type == "POSITION")
      {
		...
		this->dataPtr->controls[i].joint->SetVelocity(1, this->dataPtr->controls[2].cmd);
      }
      else if (this->dataPtr->controls[i].type == "EFFORT")
      {
        const double force = this->dataPtr->controls[i].cmd;
...
/////////////////////////////////////////////////
void ArduPilotPlugin_Rover::ReceiveMotorCommand()
{
        ...
        if (this->dataPtr->controls[i].channel < recvChannels)
        {
			...
			if(this->dataPtr->controls[i].type =="VELOCITY"){
				// bound incoming cmd between 0 and 1
          		const double cmd = ignition::math::clamp(pkt.motorSpeed[2],-1.0f, 1.0f);
          		this->dataPtr->controls[i].cmd =this->dataPtr->controls[i].multiplier *(this->dataPtr->controls[i].offset + cmd-0.5);
				gzdbg << "speed [" << i<<" ]:"<<this->dataPtr->controls[i].cmd<<"\n";
			}else if(this->dataPtr->controls[i].type =="POSITION"){
				// bound incoming cmd between 0 and 1
          		const double cmd = ignition::math::clamp(pkt.motorSpeed[0],-1.0f, 1.0f);
			    this->dataPtr->controls[i].cmd =2*this->dataPtr->controls[i].multiplier*(this->dataPtr->controls[i].offset + cmd-0.5);	
				gzdbg << "steer [" << i<<" ]:"<<this->dataPtr->controls[i].cmd<<"\n";	
			}
          //gzdbg << "apply input chan[" << i<<" ]:"<<pkt.motorSpeed[i]<<"\n";
          //       << "] to control chan[" << i
          //       << "] with joint name ["
...
/////////////////////////////////////////////////
void ArduPilotPlugin_Rover::SendState() const
{
  // send_fdm
  fdmPacket pkt;
...
*/
  if(this->dataPtr->socket_out.Send(&pkt, sizeof(pkt))!=sizeof(pkt)){
		gzerr << "send message err"<<"\n";	
  }
}
